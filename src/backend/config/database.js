...
async function initDatabase() {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      last_nickname_changed_at DATETIME NULL,
      identity_ci_hash CHAR(64) NULL,
      identity_di_hash CHAR(64) NULL,
      phone_hash CHAR(64) NULL,
      is_adult_verified TINYINT(1) NOT NULL DEFAULT 0,
      adult_verified_at DATETIME NULL,
      last_identity_verified_at DATETIME NULL,
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS posts (
      id BIGINT PRIMARY KEY AUTO_INCREMENT,
      user_id BIGINT NULL,
      board_type ENUM('FREE','ANON','REVIEW','STORY','QUESTION','EVENT','PROMOTION') NOT NULL DEFAULT 'FREE',
  ...
  if (!boardTypeColumn.length) {
    await pool.query("ALTER TABLE posts ADD COLUMN board_type ENUM('FREE','ANON','REVIEW','STORY','QUESTION','EVENT','PROMOTION') NOT NULL DEFAULT 'FREE' AFTER user_id");
  }


  await pool.query("ALTER TABLE posts MODIFY COLUMN board_type ENUM('FREE','ANON','REVIEW','STORY','QUESTION','EVENT','PROMOTION') NOT NULL DEFAULT 'FREE'");
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS point_histories (
      id BIGINT PRIMARY KEY AUTO_INCREMENT,
      user_id BIGINT NOT NULL,
      action_type ENUM('REGISTER','LOGIN_DAILY','CREATE_POST','CREATE_REVIEW_BONUS','CREATE_COMMENT','LIKE_POST','RECEIVE_POST_LIKE','REVOKE_CREATE_POST','REVOKE_CREATE_REVIEW_BONUS','REVOKE_CREATE_COMMENT','REVOKE_LIKE_POST','REVOKE_RECEIVE_POST_LIKE','ADMIN_ADJUST_ADD','ADMIN_ADJUST_DEDUCT') NOT NULL,
      points INT NOT NULL,
      reason VARCHAR(255) NULL,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  ...
  const [pointHistoryReasonColumn] = await pool.query(
    `SELECT 1
     FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = ?
  ...
  if (!pointHistoryReasonColumn.length) {
